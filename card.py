...
#this will make our deck and return the cards we need
    def deck():
    ...
    #this makes an iterative list of the keys from both x and y dictionaries simulating all cards in a deck
        ...
        #picks a random card from the random shuffled cards that are already randomly picked and returns 1
        cardList = [a,b,c,d,e,f,g,h,i,j]
    ...
    stdscr.addstr(0,2,"Press 1 to continue")
    stdscr.refresh()
    if key == ord('1'):
        stdscr.clear()
        ...
        if key1 == ord('1'):
            while rounds <= 2:
                card = deck()
                stdscr.clear()
                stdscr.addstr(0,0,"Take em ...")
                stdscr.addstr(0,2,f"{card}" )
                stdscr.refresh()
                rounds += 1
